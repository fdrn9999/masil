  ...
  const sys = {
    ...
    doyun_line(who) {
      ...
      if (l >= 60 && s < 30) {
        ...
        hint = '[힌트] 잘 보이려고만 말고, 약한 모습이나 진솔한 속얘기를 꺼내봐.';
      } else if (l < 30 && s < 30) {
        line = `형, 아직 ${n} 랑 서먹하네. 천천히 가도 돼, 일단 자주 말 걸어봐.`;
        hint = '[힌트] 아직 서먹하니까, 가벼운 얘기로 자주 말부터 걸어봐.';
      } else if (s >= 50) {
        line = `오 형, ${n} 한테 진심이 통하고 있어. 형 이대로만 가면 돼.`;
        hint = '[힌트] 결정적인 순간에 솔직하게 마음을 말해봐. 그게 제일 어렵고, 제일 커.';
      } else {
        line = `형, ${n} 랑 나쁘진 않아. 근데 더 깊어지려면 형 진짜 속얘길 해야지.`;
        hint = '[힌트] 친한 건 충분해. 이제 진짜 속얘길 보여줄 차례야.';
      }
